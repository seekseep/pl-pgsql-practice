...
## 0. 基本 LOOP (無限ループ + EXIT)

**ファイル:** `sql/2-loop/0-loop-exit.sql`
...
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/0-loop-exit.sql
```

## 1. WHILE ループ

**ファイル:** `sql/2-loop/1-while.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/1-while.sql
```

## 2. FOR ループ (整数範囲)

**ファイル:** `sql/2-loop/2-for-integer.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/2-for-integer.sql
```

## 3. FOR ループ (クエリ結果)

**ファイル:** `sql/2-loop/3-for-query.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/3-for-query.sql
```

## 4. FOR ループ (動的 SQL)

**ファイル:** `sql/2-loop/4-for-execute.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/4-for-execute.sql
```

## 5. CONTINUE (スキップ)

**ファイル:** `sql/2-loop/5-continue.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/5-continue.sql
```

## 6. EXIT (条件付き脱出)

**ファイル:** `sql/2-loop/6-exit-condition.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/6-exit-condition.sql
```

## 7. ネストしたループ

**ファイル:** `sql/2-loop/7-nested-loop.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/7-nested-loop.sql
```

## 8. ラベル付きループ

**ファイル:** `sql/2-loop/8-labeled-loop.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/3-basic-control/sql/2-loop/8-labeled-loop.sql
```

---

← [前へ: 条件分岐](1-condition.md) | [BASIC_CONTROL](README.md) | [次へ: カーソル](3-cursor.md) →
