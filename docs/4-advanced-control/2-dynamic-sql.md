...
**ファイル:** `sql/2-dynamic-sql/0-basic-execute.sql`
...
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/0-basic-execute.sql
```

## 1. USING によるパラメータバインド

**ファイル:** `sql/2-dynamic-sql/1-using-params.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/1-using-params.sql
```

## 2. テーブル名を動的に指定

**ファイル:** `sql/2-dynamic-sql/2-dynamic-table.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/2-dynamic-table.sql
```

## 3. format() の書式指定子

**ファイル:** `sql/2-dynamic-sql/3-format-specifiers.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/3-format-specifiers.sql
```

## 4. 動的 SQL で INSERT

**ファイル:** `sql/2-dynamic-sql/4-dynamic-insert.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/4-dynamic-insert.sql
```

## 5. 動的 SQL で条件を組み立てる

**ファイル:** `sql/2-dynamic-sql/5-dynamic-conditions.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/5-dynamic-conditions.sql
```

## 6. EXECUTE ... INTO STRICT

**ファイル:** `sql/2-dynamic-sql/6-execute-strict.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/2-dynamic-sql/6-execute-strict.sql
```

---

← [前へ: RETURNING INTO](1-returning-into.md) | [ADVANCED_CONTROL](README.md) | [次へ: 配列 (ARRAY) と JSON](3-array-json.md) →
