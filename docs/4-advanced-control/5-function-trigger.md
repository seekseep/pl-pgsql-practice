...
**ファイル:** `sql/5-function-trigger/0-basic-function.sql`
...
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/0-basic-function.sql
```

## 1. 引数付き関数

**ファイル:** `sql/5-function-trigger/1-args-function.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/1-args-function.sql
```

## 2. RETURNS TABLE 関数

**ファイル:** `sql/5-function-trigger/2-returns-table.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/2-returns-table.sql
```

## 3. SETOF RECORD を返す関数

**ファイル:** `sql/5-function-trigger/3-setof-record.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/3-setof-record.sql
```

## 4. updated_at 自動更新トリガー

**ファイル:** `sql/5-function-trigger/4-updated-at-trigger.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/4-updated-at-trigger.sql
```

## 5. 監査ログトリガー

**ファイル:** `sql/5-function-trigger/5-audit-trigger.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/5-audit-trigger.sql
```

## 6. 関数の削除 (DROP FUNCTION)

**ファイル:** `sql/5-function-trigger/6-drop-function.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/5-function-trigger/6-drop-function.sql
```

---

← [前へ: CTE (WITH 句) とウィンドウ関数](4-cte-window.md) | [ADVANCED_CONTROL](README.md)
