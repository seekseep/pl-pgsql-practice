...
## 0. INSERT ... RETURNING INTO

**ファイル:** `sql/1-returning-into/0-insert-returning.sql`
...
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/1-returning-into/0-insert-returning.sql
```

## 1. UPDATE ... RETURNING INTO

**ファイル:** `sql/1-returning-into/1-update-returning.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/1-returning-into/1-update-returning.sql
```

## 2. DELETE ... RETURNING INTO

**ファイル:** `sql/1-returning-into/2-delete-returning.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/1-returning-into/2-delete-returning.sql
```

## 3. RETURNING * INTO で行全体を取得

**ファイル:** `sql/1-returning-into/3-returning-rowtype.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/1-returning-into/3-returning-rowtype.sql
```

## 4. RETURNING INTO STRICT

**ファイル:** `sql/1-returning-into/4-returning-strict.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/1-returning-into/4-returning-strict.sql
```

---

← [前へ: PERFORM](0-perform.md) | [ADVANCED_CONTROL](README.md) | [次へ: 動的 SQL (EXECUTE)](2-dynamic-sql.md) →
