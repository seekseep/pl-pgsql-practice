...
**ファイル:** `sql/4-cte-window/0-basic-cte.sql`
...
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/0-basic-cte.sql
```

## 1. 複数 CTE を組み合わせる

**ファイル:** `sql/4-cte-window/1-multiple-cte.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/1-multiple-cte.sql
```

## 2. 再帰 CTE (WITH RECURSIVE)

**ファイル:** `sql/4-cte-window/2-recursive-cte.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/2-recursive-cte.sql
```

## 3. ROW_NUMBER() ウィンドウ関数

**ファイル:** `sql/4-cte-window/3-row-number.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/3-row-number.sql
```

## 4. RANK / DENSE_RANK ウィンドウ関数

**ファイル:** `sql/4-cte-window/4-rank.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/4-rank.sql
```

## 5. LAG / LEAD ウィンドウ関数

**ファイル:** `sql/4-cte-window/5-lag-lead.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/5-lag-lead.sql
```

## 6. SUM / AVG ウィンドウ関数 (累計・移動平均)

**ファイル:** `sql/4-cte-window/6-sum-avg-window.sql`
...
source .env
psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  -f docs/4-advanced-control/sql/4-cte-window/6-sum-avg-window.sql
```

---

← [前へ: 配列 (ARRAY) と JSON](3-array-json.md) | [ADVANCED_CONTROL](README.md) | [次へ: 関数 (FUNCTION) とトリガー (TRIGGER)](5-function-trigger.md) →
